...
$order_num = $start;
$list_post_cats = db_search_post_cats_by_page($value, $start, $num_per_page);

# danh mục đã đăng
$total_approved = db_num_rows("SELECT* FROM  `tbl_post_cat` Where `cat_status`= 'Approved' AND CONVERT(`title` USING utf8) LIKE '%$value%'");
# Danh mục chờ xét duyệt
$total_waitting = db_num_rows("SELECT* FROM  `tbl_post_cat` Where `cat_status`= 'Waitting...' AND CONVERT(`title` USING utf8) LIKE '%$value%'");
# Danh mục đã bị xóa
$total_trash = db_num_rows("SELECT* FROM  `tbl_post_cat` Where `cat_status`= 'Trash' AND CONVERT(`title` USING utf8) LIKE '%$value%'");
?>
                    ...
                    <form method="POST" action="?mod=post&controller=post_cat&action=apply_post_cats&page_id=<?php echo $page_num;?>">
                        ...
                        <div class="table-responsive">
                            <table class="table list-table-wp">
                                ...
                                <?php if (!empty($list_post_cats)) {
                                    $error = array();
                                    $order = 0;
                                ?>
                                <tbody>
                                    <?php foreach ($list_post_cats as $post_cat) {
                                        $order++;
                                    ?>
                                        <tr>
