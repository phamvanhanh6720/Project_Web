...
#Số bản ghi/trang
$num_per_page = 3;
# search order and get list orders
if(!empty($_GET['value'])){
    $value = $_GET['value'];
}
...
# Đơn hàng đã đăng
$total_approved = db_num_rows("SELECT* FROM  `tbl_orders` Where `order_status`= 'Approved' AND CONVERT(`order_code` USING utf8) LIKE '%$value%'");
# Đơn hàng chờ xét duyệt
$total_waitting = db_num_rows("SELECT* FROM  `tbl_orders` Where `order_status`= 'Waitting...' AND CONVERT(`order_code` USING utf8) LIKE '%$value%'");
# Đơn hàng đã bị xóa
$total_trash = db_num_rows("SELECT* FROM  `tbl_orders` Where `order_status`= 'Trash' AND CONVERT(`order_code` USING utf8) LIKE '%$value%'");
?>
        ...
        <div id="content" class="fl-right">
            ...
            <div class="section" id="detail-page">
                <div class="section-detail">
                    ...
                    <form method="POST" action="?mod=orders&controller=order&action=apply_orders&page_id=<?php echo $page_num;?>&value=<?php if(!empty($value)){ echo $value;}?>">
                        <div class="actions">
                            ...
                            <table class="table list-table-wp">
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <td><span class="tfoot-text">Số Đơn hàng</span></td>
            ...
            <div class="section" id="paging-wp">
                <div class="section-detail clearfix">
                    ...
                    <ul id="list-paging-pages" class="fl-right">
                        <?php
                            echo get_pagging($num_page, $page_num, "?mod=orders&controller=order&action=result_search&value=".$value);
                        ?>
                    </ul>
                </div>
            </div>
